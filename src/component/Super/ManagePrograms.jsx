...
const ManagePrograms = () => {
  const header = [
    ...
    { text: "Examination", dataField: "examination" },
    { text: "Registration", dataField: "registration" },
  ...
  const initialValues = {
    ...
    examination: editData?.examination || "",
    ...
    <div className="px-3">
      ...
      <Modal show={updateModal} onHide={() => setUpdateModal(false)} centered size="lg">
        <Modal.Header className="bgTheme text-light">
          <Modal.Title>{editData ? "Edit Program" : "Add Program"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Formik
          ...
          >
            {({ values, handleChange, isValid, dirty }) => (
              <Form>
                <div className="row">
                  {["category", "duration", "examination", "registration"].map((field, i) => (
      ...
      <Modal show={deleteModal.modal} centered onHide={() => setDeleteModal({ modal: false, data: "" })}>
        ...
        <Modal.Body>
          Are you sure you want to delete <b>{deleteModal.data?.category}</b>?
        </Modal.Body>
        <Modal.Footer>
  ...
  async function getList() {
    try {
      const res = await API.get("/admin/programs/listing");
  ...
  async function add(values) {
    try {
      const res = await API.post("/programs/add", values);
  ...
  async function update(values) {
    try {
      const res = await API.put("/programs/update", values);
      show({ message: res.data.message, displayClass: res.data.status });
      setUpdateModal(false);
    } catch (error) {
  ...
  async function deleteItem(item) {
    try {
      const res = await API.delete("/programs/delete", { params: { id: item.id } });
      show({ message: res.data.message, displayClass: res.data.status });
  ...
  async function toggleStatus(values) {
    try {
      const res = await API.put("/programs/disable", values);
      show({ message: res.data.message, displayClass: res.data.status });
    } catch (error) {
