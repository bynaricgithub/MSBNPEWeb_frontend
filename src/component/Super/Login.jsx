...
import { en, show } from "../../utils/Helper";
...
const Login = () => {
  ...
  return (
    <div style={{ paddingTop: "25vh" }} className="">
      <Formik
                            ...
                            <div className="row">
                              <div className="mb-3 col-12">
                                <div className="form-floating">
                                  <Field id="floatingEmail" name="email" type="text" className="form-control" placeholder="Email" />
                                  <label htmlFor="floatingEmail">Enter Email</label>
                                  <ErrorMessage name="email">{(msg) => <span className="text-danger font-12">{msg}</span>}</ErrorMessage>
                                </div>
                              </div>
                              <div className="mb-3 col-12">
                                <div className="input-group">
                                  ...
                                  <span className="input-group-text">
                                    <i
                                      ...
                                      className={`fa fa-fw ${eyeType ? "fa-eye" : "fa-eye-slash"} field-icon toggle-password`}
                                    ></i>
                                  </span>
                                </div>
                                <ErrorMessage name="password">{(msg) => <span className="text-danger font-12">{msg}</span>}</ErrorMessage>
                              </div>

                              <div className="mb-3 col-6">
                                <div className="form-floating">
                                  <Field className="form-control" id="captcha" name="captcha" type="text" placeholder="Captcha Code" />
                                  <label htmlFor="captcha">Captcha Code</label>
                                  <ErrorMessage name="captcha">{(msg) => <span className="text-danger font-12">{msg}</span>}</ErrorMessage>
                                </div>
                              </div>
                              <div className="col-6 form-floating">
                                <div className="d-flex align-items-center form-control pt-2 px-0">
                                  <img
                                  ...
                                  <button
                                    ...
                                    onClick={() => setCaptcha(generateCaptchaCode())}
                                  >
                              ...
                              <div className="text-center col-12">
                                <hr />
                                <button className="btn admin-btn-primary btn-primary w-100" type="submit">
                                  Login
                                </button>
                              </div>
                            </div>
  ...
  async function adminLogin(values) {
    ...
    try {
      ...
      if (res.data.status === "Success") {
        ...
        sessionStorage.setItem("token", JSON.stringify(res.data.data.access_token));
